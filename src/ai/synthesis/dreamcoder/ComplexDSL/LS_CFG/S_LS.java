...
public class S_LS extends S implements Node_LS, NoTerminal_LS {
    ...
    public void sample(int budget, Node_LS n) {
        ...
        if (child instanceof S_S_LS) {
            Random r = new Random();
            if (r.nextFloat() > 0.5) {
                ((S_S_LS) child).getRightS().setChild((ChildS) n);
            } else {
                ((S_S_LS) child).getLeftS().setChild((ChildS) n);
            }
        ...
        if (child instanceof If_B_then_S_else_S_LS) {
            Random r = new Random();
            if (r.nextFloat() > 0.5) {
                ((If_B_then_S_else_S_LS) child).getElse_S().setChild((ChildS) n);
            } else {
                ((If_B_then_S_else_S_LS) child).getThen_S().setChild((ChildS) n);
            }
        }
