...
import ai.core.AI;
import ai.synthesis.dreamcoder.ComplexDSL.IAs.Avaliador;
import ai.synthesis.dreamcoder.ComplexDSL.LS_CFG.Node_LS;
import ai.synthesis.dreamcoder.ComplexDSL.EvaluateGameState.SimplePlayout;
import ai.synthesis.dreamcoder.ComplexDSL.EvaluateGameState.StatisticalData;
...
public class LocalLearner implements Avaliador {
    ...
    public LocalLearner(Node_LS ind) {
        // TODO Auto-generated constructor stub
        System.out.println("CS");
        UnitTypeTable utt = new UnitTypeTable();
        tempo_ini = System.currentTimeMillis();
    ...
    public void resetListandSetCurrentSolution(Node_LS solution) {
        UnitTypeTable utt = new UnitTypeTable();        
        this.individuos.add(solution);        
        this.adv_atual.add(new Interpreter(utt, solution));
    }
    
    public List<Node_LS> getAllIndividuos() {
        return individuos;
    }

    public void recalcula(GameState gs, int max, Node_LS n) throws Exception {
        if (contain_strategy_in_individuos(n)) {
            return ;
        }
        UnitTypeTable utt = new UnitTypeTable();
        ...
        for (int i = 0; i < this.individuos.size(); i++) {
            ...
            Pair<Double, StatisticalData> aux0 = playout.run(gs, utt, 0, max, ai, ai2, false);
            Pair<Double, StatisticalData> aux1 = playout.run(gs, utt, 1, max, ai, ai2, false);
        ...
        System.out.println();
        System.out.println("List of Individuos ");
        for (Node_LS individuo : this.individuos) {
            System.out.println(individuo.translate());
        }
...
//        }
    ...
    private boolean contain_strategy_in_individuos(Node_LS n) {
        String ast_base = n.translate();
        for (Node_LS individuo : this.individuos) {
            if (individuo.translate().equals(ast_base)) {
                return true;
            }
        }
