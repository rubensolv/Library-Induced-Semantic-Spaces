...
public class PlayerAction {
    ...
    public boolean equalsFully(Object o){
        if (!(o instanceof PlayerAction)) return false;
        
        PlayerAction a = (PlayerAction)o;
        
        String at = o.toString();
        String acomp = this.toString();
        if (at.equals(acomp)) {
            return true;
        }else{
            int unAt= 0;
            int unAcomp = 0;
            for (Pair<Unit, UnitAction> action : actions) {
                unAcomp+=1;
            }
            for (Pair<Unit, UnitAction> action : a.actions) {
                unAt+=1;
            }
            if (unAt != unAcomp) {
                return false;
        ...
        //first, check if has the same number of units
